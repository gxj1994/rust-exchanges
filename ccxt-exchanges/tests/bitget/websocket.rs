//! Bitget WebSocket integration tests.
//!
//! Tests real-time data streaming via WebSocket for Bitget exchange.
...
use ccxt_exchanges::bitget::{Bitget, BitgetOptions};
use futures_util::StreamExt;
use std::time::Duration;
use tokio::time::timeout;

/// Helper to create Bitget instance for WebSocket tests
///
/// Uses production network for public API tests.
fn create_bitget_ws(use_testnet: bool) -> Bitget {
    let config = ExchangeConfig::default();
    let options = BitgetOptions {
        testnet: use_testnet, // Public APIs use production network
        ..Default::default()
    };
    Bitget::new_with_options(config, options).unwrap()
}
...
async fn test_ws_connection() {
    ...
    let exchange = create_bitget_ws(false);
    ...
    println!("Bitget WebSocket connection successful");
    ...
    match exchange.watch_ticker("BTC/USDT").await {
        Ok(mut ticker_stream) => {
            ...
            if let Some(Ok(ticker)) = ticker_item {
                ...
                // Bitget ticker可能有last或bid/ask
                assert!(
                    ticker.last.is_some() || ticker.bid.is_some() || ticker.ask.is_some(),
                    "Ticker should have price data (last, bid, or ask)"
                );
...
///
/// # Note
/// This test may time out if there's no trading activity at the time of the test.
/// Bitget trades channel requires active trading on the symbol.
#[tokio::test]
async fn test_ws_trades() {
    ...
    println!("Attempting to subscribe to trades for BTC/USDT...");
    match exchange.watch_market_trades("BTC/USDT").await {
        Ok(mut trades_stream) => {
            ...
            let trades_item = timeout(Duration::from_secs(30), trades_stream.next())
                .await
                .ok()
                .flatten();
            if let Some(Ok(trades)) = trades_item {
                ...
                println!("Trades error: {}", e);
                panic!("Trades error: {}", e);
            } else {
                println!("Trades subscription returned no data after 30s");
                println!("Note: This might be due to:");
                println!("  1. Market inactivity (no trades happening)");
                println!("  2. Incorrect channel name or instType");
                println!("  3. WebSocket connection issue");
                panic!("Trades subscription returned no data after 30s");
            }
        }
        Err(e) => {
            println!("Trades subscription failed: {}", e);
...
/// - Bitget does NOT support S1 (1-second) timeframe, only supports: 1m, 3m, 5m, 15m, 30m, 1h, 2h, 4h, 6h, 12h, 1d, 3d, 1w, 1M
...
async fn test_ws_ohlcv() {
    ...
    let exchange = create_bitget_ws(true);
    ...
    match exchange.watch_ohlcv("BTC/USDT", Timeframe::M1).await {
        Ok(mut ohlcv_stream) => {
            ...
            if let Some(Ok(ohlcvs)) = ohlcv_item {
                ...
                println!("OHLCV subscription returned no data after 90s");
            }
        }
        Err(e) => {
...
/// Helper to create Bitget instance for swap WebSocket tests
fn create_bitget_swap_ws() -> Bitget {
    let config = ExchangeConfig::default();
    let options = BitgetOptions {
...
async fn test_ws_ticker_swap() {
    ...
    let exchange = create_bitget_swap_ws();
    ...
    match exchange.watch_ticker("BTC/USDT:USDT").await {
        Ok(mut stream) => {
            ...
            // Print subscriptions to verify instType
            println!(
                "[TEST] Current subscriptions: {:?} (should contain usdt-futures)",
            ...
            match tokio::time::timeout(std::time::Duration::from_secs(15), stream.next()).await {
                Ok(Some(Ok(ticker))) => {
                    ...
                    if let Some(bid) = ticker.bid {
                        ...
                        println!(
                            "[TEST] ⚠ Warning: Bid price is None (Bitget swap ticker API may not provide this field)"
                        );
                    }
                    ...
                    if let Some(ask) = ticker.ask {
                        ...
                        println!(
                            "[TEST] ⚠ Warning: Ask price is None (Bitget swap ticker API may not provide this field)"
                        );
                    }
    ...
    match exchange.watch_order_book("BTC/USDT:USDT", Some(5)).await {
        Ok(mut stream) => {
            ...
            match tokio::time::timeout(std::time::Duration::from_secs(15), stream.next()).await {
                ...
                Ok(None) => {
                    println!("[TEST] ✗ Swap OrderBook stream ended (no more messages)");
                }
                Err(_) => {
                    println!("[TEST] ✗ Swap OrderBook timeout after 15s (no messages received)");
                    println!("[TEST] Connection state: {:?}", exchange.ws_state());
                    println!(
                        "[TEST] Current subscriptions: {:?}",
                        exchange.subscriptions()
    ...
    match exchange.watch_market_trades("BTC/USDT:USDT").await {
        Ok(mut stream) => {
            ...
            match tokio::time::timeout(std::time::Duration::from_secs(15), stream.next()).await {
                ...
                Err(_) => {
                    println!("[TEST] ✗ Swap Trades timeout after 15s (no messages received)");
                    println!("[TEST] Connection state: {:?}", exchange.ws_state());
                    println!(
                        "[TEST] Current subscriptions: {:?}",
                        exchange.subscriptions()
    ...
    match exchange.watch_ohlcv("BTC/USDT:USDT", Timeframe::M1).await {
        Ok(mut stream) => {
            ...
            match tokio::time::timeout(std::time::Duration::from_secs(90), stream.next()).await {
                ...
                Err(_) => {
                    println!(
                        "[TEST] ✗ Swap OHLCV timeout after 90s (may need to wait for next candle)"
                    );
                    println!("[TEST] Connection state: {:?}", exchange.ws_state());
                    println!(
                        "[TEST] Current subscriptions: {:?}",
