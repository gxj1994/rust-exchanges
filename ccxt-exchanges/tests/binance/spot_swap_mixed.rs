//! Binance 现货+合约混用 WebSocket 复合测试
//!
...
use ccxt_exchanges::binance::ws::create_binance_ws_client;
...
async fn test_spot_swap_mixed_ticker_and_ohlcv() {
    let client = create_binance_ws_client(false);
    ...
    // 打印用于取消订阅的 channel 信息
    let cancel_channel = SubscriptionChannel::kline("BTC/USDT", "1m");
    println!(
        "[TEST] Unsubscribe channel: type={:?}, symbol={}, params={:?}",
        cancel_channel.channel_type, cancel_channel.symbol, cancel_channel.params
    );

    client
        .unsubscribe(&[cancel_channel])
        .await
...
/// 复合测试：同时订阅现货和合约的 bids_asks (bookTicker)，验证数据 + 取消订阅
#[allow(unused)]
#[tokio::test]
async fn test_spot_swap_mixed_bids_asks() {
    ...
    // ── 1. 预连接现货和合约端点 ──
    // 注意：bookTicker 属于 /public 类别，会自动路由到 /public/ws
    let spot_ctx = WsContext::new().with_market_type(MarketType::Spot);
    let swap_ctx = WsContext::new().with_market_type(MarketType::Swap);
    ...
    assert!(
        swap_ba_after.symbol.as_str().contains("BTC"),
        "Swap bids_asks symbol should contain BTC"
    );
