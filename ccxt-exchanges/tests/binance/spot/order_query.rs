//! Binance Spot Order Query Tests
//!
...
async fn test_spot_fetch_order_market() {
    ...
    // Create market order
    let request = spot_market_buy_request("BTC/USDT", dec!(10.0)); // 10 USDT
    let order = exchange
...
async fn test_spot_fetch_open_orders() {
    ...
    let ticker = exchange
        .fetch_ticker(
            "BTC/USDT",
            ccxt_core::types::common::ticker_params::TickerParams::default(),
        )
        .await
...
async fn test_spot_fetch_history_orders() {
    ...
    for i in 0..3 {
        let request = spot_market_buy_request("BTC/USDT", dec!(10.0));
        let order = exchange
            .create_order(request)
            .await
            .expect("Failed to create market order");
        println!("✅ Created market order {}: {}", i + 1, order.id);
        filled_ids.push(order.id);
        tokio::time::sleep(Duration::from_secs(1)).await;
    }

    // 等待测试网同步订单数据
    println!("⏳ Waiting for testnet to sync order history...");
    tokio::time::sleep(Duration::from_secs(3)).await;

    // Fetch history orders - 扩大时间范围到 24 小时
    let since = chrono::Utc::now().timestamp_millis() - 86400000; // 24 hours ago
    let history_orders = exchange
        .fetch_history_orders(Some("BTC/USDT"), Some(since), Some(50))
        .await
        .expect("Failed to fetch history orders");

    println!("📊 Fetched {} history orders", history_orders.len());

    assert!(
        !history_orders.is_empty(),
        "Should have at least one history order"
    );
    ...
    for filled_id in &filled_ids {
        if history_ids.contains(&filled_id) {
            println!("   ✅ Found order: {}", filled_id);
            found_count += 1;
        } else {
            println!("   ⚠️  Order not in history: {}", filled_id);
        }
    }

    // 注意：测试网可能有延迟，不强制要求所有订单都立即出现在历史记录中
    // 只要至少找到一个订单就说明 API 工作正常
    assert!(
        found_count > 0,
        "At least one order should be in history (found {} out of {})",
        found_count,
        filled_ids.len()
    );

    println!(
        "✅ History order query test passed ({}/{} orders found)",
        found_count,
        filled_ids.len()
    );
}
...
async fn test_spot_market_sell_order() {
    ...
    let buy_request = spot_market_buy_request("BTC/USDT", dec!(10.0));
