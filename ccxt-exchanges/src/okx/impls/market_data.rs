...
impl MarketData for Okx {
    ...
    async fn fetch_order_book_with_params(
        ...
        Okx::fetch_order_book(self, symbol, params.limit).await
    }
    ...
    async fn fetch_ohlcv_with_params(
        ...
        let ohlcv_data = Okx::fetch_ohlcv(self, request).await?;
...
mod tests {
    ...
    #[test]
    fn test_okx_market_data_trait_object_safety() {
        let okx = Okx::new(ExchangeConfig::default()).unwrap();
        let market_data: Box<dyn MarketData> = Box::new(okx);

        assert_eq!(market_data.id(), "okx");
