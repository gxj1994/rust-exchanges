...
impl MarketData for Bitget {
    async fn fetch_markets(&self) -> Result<Vec<Market>> {
        let markets = Bitget::fetch_markets(self).await?;
    ...
    async fn fetch_order_book_with_params(
        ...
        Bitget::fetch_order_book(self, symbol, params.limit).await
    }
    ...
    async fn fetch_ohlcv_with_params(
        ...
        let ohlcv_data = Bitget::fetch_ohlcv(self, request).await?;
...
mod tests {
    ...
    #[test]
    fn test_bitget_market_data_trait_object_safety() {
        let bitget = Bitget::new(ExchangeConfig::default()).unwrap();
        let market_data: Box<dyn MarketData> = Box::new(bitget);

        assert_eq!(market_data.id(), "bitget");
