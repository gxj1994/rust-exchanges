...
impl MarketData for Binance {
    ...
    async fn fetch_order_book_with_params(
        ...
        Binance::fetch_order_book(self, symbol, params.limit).await
    }
    ...
    async fn fetch_ohlcv_with_params(
        ...
        let ohlcv_data = Binance::fetch_ohlcv(self, request).await?;
        ...
        ohlcv_data
            ...
            .map(|o| -> ccxt_core::Result<Ohlcv> {
                Ok(Ohlcv {
                    ...
                    open: ccxt_core::Price(Decimal::try_from(o.open).map_err(|e| {
                        ccxt_core::Error::from(ccxt_core::ParseError::invalid_value(
                            "OHLCV open",
                            format!("{e}"),
                        ))
                    })?),
                    high: ccxt_core::Price(Decimal::try_from(o.high).map_err(|e| {
                        ccxt_core::Error::from(ccxt_core::ParseError::invalid_value(
                            "OHLCV high",
                            format!("{e}"),
                        ))
                    })?),
                    low: ccxt_core::Price(Decimal::try_from(o.low).map_err(|e| {
                        ccxt_core::Error::from(ccxt_core::ParseError::invalid_value(
                            "OHLCV low",
                            format!("{e}"),
                        ))
                    })?),
                    close: ccxt_core::Price(Decimal::try_from(o.close).map_err(|e| {
                        ccxt_core::Error::from(ccxt_core::ParseError::invalid_value(
                            "OHLCV close",
                            format!("{e}"),
                        ))
            ...
            .map_err(|e| e.context("Failed to convert Binance OHLCV data"))
    }
}
...
mod tests {
    ...
    #[test]
    fn test_binance_market_data_trait_object_safety() {
        let binance = Binance::new(ExchangeConfig::default()).unwrap();
        let market_data: Box<dyn MarketData> = Box::new(binance);

        assert_eq!(market_data.id(), "binance");
